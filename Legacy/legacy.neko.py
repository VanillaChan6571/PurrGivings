...
class GiveawayBot(discord.Client):
    def __init__(self):
        ...
        self.owner_id = None  # Store the owner's ID
    ...
    async def on_ready(self):
        ...
        # Fetch the application info to get the owner's ID
        app_info = await self.application_info()
        self.owner_id = app_info.owner.id
        logger.info(f"Bot owner ID: {self.owner_id}")
    ...
    async def set_status(self, interaction: discord.Interaction, status_type: str):
        if status_type not in self.status_config:
            await interaction.response.send_message(
                f"Invalid status type. Choose from: {', '.join(self.status_config.keys())}", ephemeral=True)
            return

        await self.update_status()
        await interaction.response.send_message(f"Status updated to '{status_type}' type.", ephemeral=True)

    @discord.app_commands.command(name="set_avatar", description="Set a new avatar for the bot (Owner Only)")
    @app_commands.describe(
        avatar_input="URL of the image or upload an image as an attachment"
    )
    async def set_avatar(self, interaction: discord.Interaction, avatar_input: str = None):
...
class GiveawayView(discord.ui.View):
    ...
    @discord.ui.button(label="Pray for Gods of Nekos", style=discord.ButtonStyle.primary,
                       emoji="<:Peek:1222014873735790644>")
